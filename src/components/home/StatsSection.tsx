        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_0.95fr] overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] border border-white/10 bg-white shadow-2xl">
          <div className="p-8 sm:p-10 md:p-14 lg:p-16">
            <div className="mb-10 md:mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-sans font-black text-white tracking-tighter uppercase italic">Journeys made memorable</h2>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-7 sm:gap-x-5 sm:gap-y-8 md:gap-x-10 md:gap-y-10">
              {stats.map((stat, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }} viewport={{ once: true }} className="flex min-w-0 items-center gap-2.5 sm:gap-3 md:gap-4 group cursor-default">
                  ...
                  <div className="min-w-0"><p className="text-xl sm:text-2xl md:text-4xl font-sans font-black text-white tracking-tighter leading-none uppercase italic liquid-text">{stat.value}</p><p className="text-[7px] sm:text-[8px] md:text-[9px] text-white/40 uppercase tracking-[0.12em] sm:tracking-[0.2em] md:tracking-[0.3em] font-black mt-1.5 md:mt-2 whitespace-normal leading-tight">{stat.label}</p></div>
                </motion.div>
              ))}
            </div>
          </div>

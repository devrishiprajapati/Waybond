  ...
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen pt-40 pb-24 bg-charcoal text-white relative overflow-hidden">
    ...
    <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20 relative z-10">
      <div className="max-w-4xl mb-16 md:mb-20 space-y-6">
        ...
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-5xl md:text-7xl lg:text-[6rem] font-display font-black text-white tracking-tighter uppercase leading-[0.9] liquid-text italic">
        ...
        {guidelines.map((guideline, index) => {
          ...
          return <motion.article key={guideline.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.08 }} className={`liquid-glass-dark p-8 md:p-10 rounded-[2.5rem] border border-white/10 shadow-xl group hover:border-secondary/30 transition-colors ${index === guidelines.length - 1 ? 'md:col-span-2' : ''}`}>
            <div className="flex gap-6 items-start">
              ...
              <div><p className="text-[9px] font-black tracking-[0.3em] text-secondary uppercase mb-2">Guideline {String(index + 1).padStart(2, '0')}</p><h2 className="text-2xl md:text-3xl font-display font-black uppercase italic tracking-tighter text-white">{guideline.title}</h2><p className="text-sm text-white/55 leading-relaxed font-medium italic mt-4 max-w-2xl">{guideline.description}</p></div>
            </div>
          </motion.article>
        })}
      </div>
      <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-10 liquid-glass p-8 md:p-12 rounded-[3rem] border border-white/15 shadow-2xl text-center">
        <h2 className="text-3xl md:text-4xl font-display font-black uppercase italic tracking-tighter text-white">In an emergency</h2>

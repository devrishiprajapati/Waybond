...
const emptyCard = (): TrendingCard => ({ image: '', badge: 'Adventure', title: '', subtitle: '' })
...
export default function EditHero() {
  ...
  useEffect(() => {
    ...
    getTrendingCards().then((loadedCards) => setCards(loadedCards.slice(0, 6)))
  }, [navigate])

  const updateCard = (index: number, field: 'badge' | 'title' | 'subtitle', value: string) => {
  ...
  const handleSave = async (event: React.FormEvent) => {
    ...
    if (!cards.length || cards.length > 6 || cards.some((card) => !card.image || !card.title.trim() || !card.subtitle.trim())) {
      setError(cards.length > 6 ? 'A maximum of 6 Trending Adventure cards is allowed.' : 'Every card needs an image, title, and subtitle.')
      return
    }
    ...
    setError('')
    try {
      await updateTrendingCards(cards)
      navigate('/admin/dashboard')
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : 'Unable to save Trending Adventures.')
    } finally {
      setSaving(false)
    }
  }

...
const AdminDashboard = () => {
  ...
  const navItems = [
    ...
    { label: 'Cancellations', path: '/admin/cancellations', icon: XCircle, permission: 'view_bookings' },
    { label: 'Enquiries', path: '/admin/enquiries', icon: MessageCircle, permission: 'view_bookings' },
  ...
  return (
    <>
      <PermissionGuard>
        <div className="min-h-screen bg-white text-white flex overflow-x-hidden pb-24 lg:pb-0">
          ...
          <main className="min-w-0 flex-grow p-4 pt-24 sm:p-6 sm:pt-24 md:p-10 lg:p-12 lg:pt-12 overflow-y-auto">
            <div className="mb-6 flex items-center justify-between gap-3 rounded-2xl border border-white/25 bg-gradient-to-br from-[#004AAD] via-[#1d6fe8] to-[#6495ED] px-4 py-3 shadow-xl shadow-secondary/20 lg:hidden">
              <div className="min-w-0">
                <p className="text-[9px] font-black uppercase tracking-[0.28em] text-white/80">WayBond</p>
                <p className="truncate text-sm font-black uppercase text-white">Admin Basecamp</p>
              </div>
              <button
                onClick={handleLogout}
                className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/20 bg-white/15 text-white shadow-sm transition-colors hover:bg-white/25"
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-5 mb-4 md:mb-8">
              {stats.map((stat, index) => (
                <motion.div
                  ...
                  className="liquid-glass-dark border border-white/10 rounded-xl lg:rounded-[2rem] p-3 md:p-6 flex flex-row lg:flex-row items-center gap-2 md:gap-5"
                >
                  <div className="w-9 h-9 md:w-14 md:h-14 rounded-lg lg:rounded-2xl bg-secondary/15 border border-secondary/20 flex items-center justify-center text-secondary shrink-0">
      ...
      {deleteConfirmId !== null && (() => {
        const trip = trips.find(t => t.id === deleteConfirmId)
        return (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-grow min-w-0">
                  <p className="text-sm font-black uppercase tracking-wide text-white font-bungee">Delete Package?</p>
                  {trip && (
                    <p className="text-xs text-white/45 font-medium mt-1 truncate">{trip.title}</p>
                  )}

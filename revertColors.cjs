...
function walk(dir) {
    ...
    list.forEach(file => {
        ...
        } else if (file.endsWith('.tsx') && !file.endsWith('Navbar.tsx')) {
            results.push(file);
        }
    });
    return results;
...
files.forEach(file => {
    ...
    // Reverse text-slate-900 back to text-white
    content = content.replace(/text-slate-900\//g, 'text-white/');
    content = content.replace(/text-slate-900/g, 'text-white');
    content = content.replace(/text-slate-800/g, 'text-white');

    // Reverse bg-white back to bg-charcoal, but carefully
    // Replace bg-white that is NOT followed by /
    content = content.replace(/bg-white\b(?!\/)/g, 'bg-charcoal');
    // Fix the accidental hover:bg-charcoal back to hover:bg-white (which were buttons originally)
    content = content.replace(/hover:bg-charcoal/g, 'hover:bg-white');

    fs.writeFileSync(file, content, 'utf8');
});

console.log('Successfully reverted colors in React components');
